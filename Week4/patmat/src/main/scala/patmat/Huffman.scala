...
object Huffman {
  ...
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(_, _, charsList, _) => charsList
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    def times(bag: List[(Char, Int)], chars: List[Char]): List[(Char,Int)] = chars match {
      case List() => bag
      case x :: xs => times(insert(bag, x), xs) 
    }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def insert(bag: List[Leaf], item: Leaf) : List[Leaf] = bag match {
      case List() => item :: Nil
      case x::xs  => if (weight(x) >= weight(item)) item :: bag else x :: insert(xs, item)
    }
    freqs match {
      case List() => Nil
      case (x, y) :: xs => insert(makeOrderedLeafList(xs), Leaf(x, y))
    }
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case List() => throw new Exception("codeBits")
    case (`char`, bitList) :: xs => bitList
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def innerQuickEncode(table: CodeTable, text: List[Char]): List[Bit] = text match {
      case List() => List()
      case x :: xs => {
        val item = table.find( _._1 == x)
        if (!item.isEmpty) item.get._2 ::: innerQuickEncode(table, xs)
        else throw new Exception("quickEncode")
      }
    }
  
    innerQuickEncode(convert(tree), text)
  }
}
